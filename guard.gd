...
func _fixed_process(delta):
	...
	var player = nav2d.get_node("Player").get_node("AnimatedSprite")
	var child = get_child(0)

	nav2d.path_to_paint = null
	...
	var close_enough = player.get_global_pos().distance_to(child.get_global_pos()) < 2 * 64
	if close_enough:
		...
		if dir == 1 and (angle > 315 or angle < 45):
			good_angle = true
		elif dir == 2 and angle > 45 and angle < 135:
			good_angle = true
		elif dir == 3 and angle > 135 and angle < 225:
			good_angle = true
		elif dir == 0 and angle > 225 and angle < 315:
			good_angle = true
		...
		if good_angle:
			...
			if path.size() == 2:
				...
				print("angle is good, but i cant see the player.")
				if path != null:
					nav2d.path_to_paint = path
